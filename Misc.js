function createChild(options) {
    const separatelyHandledOptions = `className text tagName`.split(' ')
    ...
    Object.keys(options)
        .filter((optionName) => !separatelyHandledOptions.includes(optionName))
        .forEach((optionName) =>
            node.setAttribute(optionName, options[optionName])
        )
